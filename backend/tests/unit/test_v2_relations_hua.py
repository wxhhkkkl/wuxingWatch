...
def test_tou_scope_participants_for_liuhe():
    ...
    己亥 丙寅 辛酉 乙丑：月令寅（木当令，条件② 满足）；寅、亥两参与支的同柱天干
    为 己（土）、丙（火），均非木，故「在其上透出」不成立；全局地支木 < 26
    → 条件③ 不满足 → 寅亥六合按合绊。时干乙虽为木，但它不在参与支上，不算数。

    > 原用例（壬寅 己亥 乙卯 丁未）含亥卯未三合，三合化成功会把全局木抬到 26 以上，
    > 于是寅亥六合**确实应该化**（书 上 3480/2553 的级联）——该例移到
    > `test_hua_degree_counts_other_relations_effects`，本用例改用无局的盘。
    """
    r = relations.judge_relations(_chart("己亥", "丙寅", "辛酉", "乙丑"))
    liuhe = [e for e in r["established"] if e["tier"] == 12]
    assert liuhe, "寅亥六合仍应成立（合绊）"
    assert liuhe[0]["hua"] is None, "他柱（时干乙）透木不算数，故不化"


def test_hua_degree_counts_other_relations_effects():
    """条件③ 的度数须计入**其它已成立关系**的 effects（级联，书 上 3480 / 2553）。

    壬寅 己亥 乙卯 丁未：亥卯未三合木（tier 6）先成立且化成功 → 亥卯未变纯木
    （每支 6 度）→ 全局地支木随之达到太旺以上 → 与之**并存**的寅亥六合（tier 12，
    化神同为木、共用亥）条件③ 由「度数达标」这一支满足 → 也化成功。

    书证同构两处：
    - 上 3480「亥卯未合化木成功——三支都变成了木，每支含木6度…再乘以月令的系数2，
      就变为36度。**地支木达到太旺以上，导致卯辰半会木也成功**」（两关系共用卯）；
    - 上 2553「逢92壬申年，申子辰合水成功，**导致了地支水达到了太旺以上，所以引发了
      子丑合化水也成功**，与申子辰合化水并存」。

    > 判定时**不计本关系自己**的 effects（否则化成功→整支变纯化神→抬高自己的化神度数，
    > 条件③ 自我实现）；共用柱位的其它关系照计——上两例的并存关系都共用同一支。
    """
    r = relations.judge_relations(_chart("壬寅", "己亥", "乙卯", "丁未"))
    sanhe = _estab(r, 6)
    liuhe = _estab(r, 12)
    assert sanhe and sanhe[0]["hua"] == "木", "亥卯未三合木应化成功（先成立）"
    assert liuhe and liuhe[0]["hua"] == "木", "三合把全局木抬过 26 → 寅亥六合也随之化"
