...
- [X] T026 [US2] 大运之干与流年之干纳入天干层面：`backend/src/services/bazi/v2/stem_he.py`（现 `:26` 明文排除）——含换字后重判地支（FR-012a / FR-014a），且「不与原局之干争合」而「两岁运之干之间可争合」（FR-012 / FR-014a）——依赖 T020
  ...
  >
  > **⚠️ 登记一处有意的未尽（R13）**：岁运之干若参与五合且化成功，其**换字**（如运干甲→戊）
  > **未**传到 `dayun.apply_dayun_delta` 的「运干同类 +1」判据上。`he["hua"]` 已按扩展下标
  > 带回新干、接线是现成的，但**书里没有这一层的书证**：书 上 1745 是唯一直接论「运干参与五合」
  > 的算例，其运干为己土——恰是**换字表里不换字的那两个之一**，故该例验不到。
  > 沿 T017 的先例，此处**只登记不实现**，位置（`pipeline.py` 的 `_sy_cols` 与 `he["hua"]` 交界）
  > 与**实算的影响面**（十个干位里八个会换字）见 research.md R13。
- [X] T027 [US2] 流年进入三段编排
...
- [X] T047 跑全量 **12⁴ 影响面**（`backend/src/scripts/sweep_impact.py`），把六个指标的变动盘数与占比写入 `specs/013-dayun-liunian-judgment/research.md`（沿 012 期 O-6~O-9 的格式）
  ...
  > 另把**岁运的作用面**也测了（阶段 1→2、阶段 2→3 各一张表）：关系判定 97.37% / 100.00%，
  > 动态旺度 100.00% / 91.61%，日主等级 35.08% / 37.28%，格局 15.88% / 12.92%，
  > 喜忌 28.43% / 22.50%。两节写在 research.md 的「12⁴ 全量影响面」里并**标明不可混读**——
  > 回归面必须为 0，作用面本就不该是 0（它的用处是**证伪**：某指标若为 0% 才说明接线没生效）。
