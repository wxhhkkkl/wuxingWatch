...
## 7a. 命盘快照里的岁运两列（2026-09-22 补遗；FR-027）

`steps[].chart` / `steps[].charts[].chart` 的 `pillars` 在**岁运端点**下另含两列
（形状见 012 期 data-model §7；本节的增量只在此）：

| 字段 | 岁运两列（`_dayun` / `_liunian`）的取值 | 与四柱的差别 |
|---|---|---|
| `key` / `label` | `_dayun`→`大运`、`_liunian`→`流年` | 四柱是 `year…time` / `年…时` |
| `gan_degree` | 恒 `1`；合而不化者 × 成数（0.6 / 0.8…） | 四柱第 4 段起是本柱所在**连片组**的旺度 |
| `gan_own` / `gan_root` | **恒 `null`** | 四柱第 4 段起给出，且恒有 `主数 = 自身 + 根` |
| `gan_original` / `gan_change` | 该干参与五合时给（`合化` 带原字、`合绊` 不带） | 同 |
| `hidden[].degree` | 临大运/临流年**独立档**，**平加**（不乘月令系数） | 四柱按 `static`/`dynamic` 段乘月令系数 |
| `hidden[].change` | **恒 `null`**（不与原局藏干表比对） | 四柱给 `新增/归零/增力/减力/变纯` |
| `zhi_effective_wx` | 恒等于本气五行（岁运之支不参与原局的合化变纯） | 四柱可被合化改宗 |
| 列的位置 | 数组里**追加在四柱之后**，显示时前端重排到**最左** | —— |

**何时有哪一列**：`_dayun` 阶段 2/3 恒有；`_liunian` **仅阶段 3 且未被门控挡住时**。
被挡住时后端**不建**那一列，前端出灰显占位（判据只能是「有没有这一列」——
`analyze_step` 的门控不清 `item["liunian"]`）。

> **⚠️ 已知简化，勿当成可加总的账**：岁运列只给「这一列自己」的度数——天干那 1 度是
> 「同类相助 +1」、地支那份是平加的藏干。它**不等于**页面上方 `scores_after` 里的五行值
> （后者另含运支状态增减 ±2/±1.5 等）。两套数并排出现时不要写任何「应相等」的断言或文案。
>
> **入库路径有意不加列**：`analyze_all` → `strength.dayun[]` 的 64 张快照仍为四柱
> （`chart_result` 已是 MEDIUMTEXT），见 [contracts/suiyun-v2.md](contracts/suiyun-v2.md) §3。
