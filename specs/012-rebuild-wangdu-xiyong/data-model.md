...
| `stem_he` | object | 第 6 段天干五合的结构化结论：`{hua, ban, ban_cheng, blocked, established, traces}` | — |
...
| `gan_degree` | **主数**。第 1–3 段为该字**自身**的生度数（原局 1）；**第 4 段（通根递减）起改为该字所在连片组的旺度**——即生克算式里真正用的那个数（含通根，书 上 771/744） |
| `gan_own` / `gan_root` | 主数的两个分量：`gan_own` = 该字自身旺度（生度数 × 月令系数），`gan_root` = 该组的根。恒有 **主数 = 自身 + 根**、根 ≥ 0（组被抽到比自身还低时，自身跟着见底）。第 1–3 段为 null |
| `zhi_effective_wx` | 该支**实际承载**的五行——合化变纯时取化神，否则为本气五行。前端据此上色并标「变X」 |
| `hidden[].change` | 相对**原始藏干表**的变化：`新增`／`归零`／`增力`／`减力`／`变纯`／null。基准恒为原始表，故出标记的各段（第 2–5 段与第 7 段起）一致（第 1 段「原局」与第 6 段「五合」不出） |
...
| 2 关系对藏干度数的影响 / 3 月令系数 | 同上 | 关系影响后 |
| 4 通根递减 | **连片组旺度**（= 自身 + 根，`gan_own`/`gan_root` 同时给出） | 藏干 × 月令系数 |
| 5 静态旺度（原字） | 同第 4 段 | 同上 |
| 6 天干五合（换字 + 合绊减力） | 换字后 + 合绊（按**整组静态旺度**缩放） | 同上 |
| 7 换字后重算静态（**仅真有换字时**） | 同第 6 段 | 同上 |
| 8 生克结算 / 9 动态旺度与定级 | 结算当下 | 本气取该支本气实例终值；中余气取 藏干 × 系数 |

> 段数是**动态**的：有合化换字时 9 段（含第 7 段），只合绊或无五合时 8 段。
> 其后另接包装层的 `geju` / `yongshen` 两段，段号由 `len(旺度段)+1` 续排。

**逐实例快照**（生克结算段专有）：`steps[].charts` = `[{label, after, chart}]`——结算过程中
每完成一个实例（连片天干组 / 同柱本气）记一张，`after` 是该次结算完时依据行已产出的
条数，前端据此把图**插在对应算式之后**；末尾必有一张 `label = "本段结算完成"` 的收尾图
（即该段终态），故该段不再另贴段末快照。

**已知简化（非缺陷、但不静默）**：通根的「按最近一支递减一次」是整段扣减，归不到单个藏干头上，故第 5 段（静态旺度）起同一支内各藏干度数之和会略大于 `degrees[wx].root` 中该支的贡献。快照不给该合计，`degrees` 契约不受影响。
